           ...
           # create preview 
            ...
            if exe_query:
                try:
                    ...
                    db = DBConnection(db_config,1,1)
                    db.execute_query(sql_query)
                    if result is None:
                        return ProcessResult(success=False, message="Query execution failed")
                except Exception as e:
                    return ProcessResult(success=False, message=f"Database error: {e}")
                finally:
                    if db and db._pool:
                        db.close_pool()
